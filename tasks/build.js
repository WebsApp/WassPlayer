...
gulp.task('clean', done => {
  const dirs = [paths.wassPlayer.output].map(dir => path.join(dir, '**/*'));
  // Don't delete the mp4
  dirs.push(`!${path.join(paths.wassPlayer.output, '**/*.mp4')}`);
  del(dirs);
  done();
});
...
gulp.task('watch', () => {
  // wassPlayer core
  gulp.watch(paths.wassPlayer.src.js, gulp.parallel('js'));
  gulp.watch(paths.wassPlayer.src.sass, gulp.parallel('css'));
  gulp.watch(paths.wassPlayer.src.sprite, gulp.parallel('sprites'));
});
